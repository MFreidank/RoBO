...
# XXX: Make this a sacred experiment

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--acquisition", default="log_ei")
    parser.add_argument("--maximizer", default="random")

    # XXX: Subcommand for each method
...
if method == "entropy_search":
    ...
    results = bohamiann(f, bounds[:, 0], bounds[:, 1],
                        num_iterations=n_iters,
                        n_init=n_init)
elif method == "keras_bohamiann":
    results = keras_bohamiann(f, bounds[:, 0], bounds[:, 1],
                              num_iterations=n_iters,
                              n_init=n_init)
